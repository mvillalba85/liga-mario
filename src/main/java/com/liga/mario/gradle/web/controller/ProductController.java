...
import com.liga.mario.gradle.domain.service.ProductService;
...
public class ProductController {
    ...
    @ApiOperation("Search products by a category ID")
    public ResponseEntity<List<Product>> getByCategory(@ApiParam(value = "The id of the category", required = true, example = "1") @PathVariable("categoryId") int categoryId){
    ...
    public Optional<List<Product>> getByCategoriaOrderByNombre(int categoryId){
        return productService.getByCategoriaOrderByNombre(categoryId);
    }

    public Optional<Product> getByNombre(String name){
        return productService.getByNombre(name);
    }
